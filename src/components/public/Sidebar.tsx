...
// Save this as: src/components/public/PublicSidebar.tsx
...
export function PublicSidebar() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [sectionsRes, adminRes, profileRes] = await Promise.all([
  ...
  return (
    <aside className="w-48 bg-[#0a0a0a] min-h-screen fixed left-0 top-0">
      <div className="p-6">
        ...
        <div className="mb-8">
          <Link href="/" className="flex items-center gap-2 hover:opacity-70 transition">
            ...
            <span className="text-white text-[14px] font-normal">{adminName}</span>
          </Link>
        </div>
      ...
      {/* Social Links at Bottom - Only on Home Page */}
      {isHomePage && profile && (profile.xLink || profile.linkedinLink || profile.instagramLink) && (
