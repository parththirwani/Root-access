...
export function SidebarClient({ layoutData }: { layoutData: LayoutData }) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex md:flex-col w-48 bg-[#0a0a0a] min-h-screen fixed left-0 top-0">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <nav className="space-y-6">
            ...
            <div>
              <Link 
                href="/" 
                className="flex items-center gap-2 text-[13px] text-white hover:opacity-70 transition"
              >
            ...
            {layoutData.sections.map((section) => (
              <div key={section.id}>
                <button
                  ...
                  className="flex items-center justify-between w-full text-[13px] text-white hover:opacity-70 transition mb-2.5"
                >
                    ...
                    {section.subsections.map((subsection) => (
                      <li key={subsection.id}>
                        <Link
                          ...
                          className="text-[13px] text-white hover:opacity-70 transition flex items-center gap-2 group"
                        >
      ...
      <button
        ...
        className="md:hidden fixed top-4 right-4 z-50 w-10 h-10 bg-[#101010] border border-[#2a2a2a] rounded-lg flex items-center justify-center text-white hover:bg-[#1a1a1a] transition cursor-pointer"
      >
        {mobileMenuOpen ? (
      ...
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-16 right-4 z-50 w-64 max-h-[80vh] overflow-y-auto bg-[#101010] border border-[#2a2a2a] rounded-xl shadow-2xl">
          <div className="p-4">
            {/* Home Link */}
            <Link 
              href="/" 
              onClick={() => setMobileMenuOpen(false)}
              className="flex items-center gap-2 px-3 py-2 text-[13px] text-white hover:opacity-70 hover:bg-[#1a1a1a] rounded-lg transition"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
              </svg>
            ...
            {/* Divider */}
            <div className="my-3 border-t border-[#2a2a2a]"></div>

            {/* Sections */}
            {layoutData.sections.map((section) => (
              <div key={section.id} className="mb-3">
                <div className="px-3 py-1 text-[11px] text-neutral-400 uppercase tracking-wider font-medium">
                  {section.name}
                </div>
                <div className="space-y-1 mt-1">
                  {section.subsections.map((subsection) => (
                    <Link
                      ...
                      href={`/${subsection.slug}`}
                      onClick={() => setMobileMenuOpen(false)}
                      className="flex items-center gap-2 px-3 py-2 text-[13px] text-white hover:opacity-70 hover:bg-[#1a1a1a] rounded-lg transition group"
                    >
                      <span className="opacity-40 group-hover:opacity-60 transition grayscale">
                        {subsection.icon}
                      </span>
